...
app.controller("MainCtrl", [
  ...
  function(
    ...
    $rootScope.user_levels = {
      2: "USER_GUEST_READER",
      3: "USER_GUEST_FOTOS",
      4: "USER_GUEST_READER_PARTNER",
      8: "Administrator",
      9: "Superadmin"
    };

    if (
      typeof $window.sessionStorage.user_data !== "undefined" &&
    ...
    $rootScope.updatePendingRequests = function() {
      if ($rootScope.user_data && $rootScope.user_data.level >= 8) {
        $http
          .get($rootScope.ApiUrl + "/?a=request&action=pendingAmount")
    ...
    $rootScope.doLogout = function() {
      $http
        .get($rootScope.ApiUrl + "/?a=user&b=logout")
        .success(function(data) {
          var resp = data;
    ...
    $scope.setLanguage = function(lang) {
      ...
      if ($state.includes("aboutFotoch") || $state.includes("contact")) {
        // Reload content after switching language
...
angular
  .module("fotochWebApp")
  .service("TranslationService", function($http, $rootScope) {
    var getTranslation = function() {
      return $http
        .get($rootScope.ApiUrl + "/?a=sprache&lang=" + $rootScope.lang)
...
app.controller("InstitutionCtrl", [
  ...
  function(
    ...
    function setMeta() {
      ...
      ngMeta.setTag(
        "description",
        $rootScope.translations.institution_description
      );
    }

    // wait for translations to be loaded before setting title
    if (!$rootScope.translationLoaded) {
      var cancelWatcher = $rootScope.$watch("translationLoaded", function(
        newValue,
        oldvalue
      ) {
        if (newValue) {
...
app.controller("InventoryCtrl", [
  ...
  function(
    ...
    function setMeta() {
      ...
      ngMeta.setTag(
        "description",
        $rootScope.translations.inventory_description
      );
    }
...
app.controller("StaticPageCtrl", [
  ...
  function(
    ...
    function loadContent() {
      $http
        .get($rootScope.ApiUrl + "/?a=pages&lang=" + $rootScope.lang)
        .success(function(data) {
          $scope.pages = data;
        });
    }
...
app.controller("ExhibitionCtrl", [
  ...
  function(
    ...
    function setMeta() {
      ...
      ngMeta.setTag(
        "description",
        $rootScope.translations.exhibition_description
      );
    }
...
app.controller("PhotographerCtrl", [
  ...
  function(
    ...
    function setMeta() {
      ...
      ngMeta.setTag(
        "description",
        $rootScope.translations.photographer_description
      );
    }
...
app.controller("LiteraturCtrl", [
  ...
  function(
    ...
    function setMeta() {
      ...
      ngMeta.setTag(
        "description",
        $rootScope.translations.literature_description
      );
    }
...
app.controller("HomeCtrl", [
  ...
  function(
    ...
    function getHeaderImage() {
      ...
      $scope.imgURL =
        "assets/img/home-intro/header-" + zeroFill(imageNo, 3) + ".jpg";
    }
        ...
        .success(function(data) {
          ...
          if (status == "ok") {
            ...
            if (data.inst_comment != "") {
              DetailService.getInstitute(data.inst_comment).then(function(
                institute
              ) {
                $rootScope.user_data.institute = institute.data;
                $window.sessionStorage.user_data = JSON.stringify(
                  $rootScope.user_data
                );
              });
            }
...
app.controller("homeSearch", [
  ...
  function($scope, $http, $location, $state, $stateParams, $rootScope, $q) {
    ...
    $scope.setIdArrays = function() {
      ...
      for (var index = 0; index < parts.length; index++) {
        ...
        if (typeof $scope.result[parts[index] + "_results"] !== "undefined") {
          for (
            var item = 0;
            item < $scope.result[parts[index] + "_results"].length;
            item++
          ) {
            $scope.result[parts[index] + "_ids"].push(
              $scope.result[parts[index] + "_results"][item].id
            );
          }
        }
      }
        ...
        $http({
          ...
          onProgress: function(event) {
            try {
              ...
              console.log(
                "Invalid response: " + event.currentTarget.responseText
              );
              console.log(e);
            }
          }
    ...
    $scope.blur = function() {
      if (
        typeof $scope.user.query !== "undefined" &&
        $scope.user.query.length > 0
      ) {
        $scope.searchActive = true;
      } else {
        $scope.searchActive = false;
