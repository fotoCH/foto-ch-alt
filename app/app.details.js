    ...
    switch (params.type) {
      case "photo":
        DetailService.getPhoto(params.id).then(function(photo) {
          ...
          ngMeta.setTag(
            "ogImage",
            $rootScope.imageRootUrl + photo.data.image_path
          );
        });
      ...
      case "photographer":
        DetailService.getPhotographer(params.id).then(function(photographer) {
          $scope.getContent = function(attribute_string) {
            if (
              $scope.detail[attribute_string + "_s"] != null &&
              typeof $scope.detail[attribute_string + "_s"][
                $scope.contentLanguage
              ] !== "undefined"
            ) {
              return $scope.detail[attribute_string + "_s"][
                $scope.contentLanguage
              ];
...
angular
  .module("fotochWebApp")
  .service("DetailService", function($http, $rootScope) {
    ...
    function getPhoto(id) {
      return $http
        .get($rootScope.ApiUrl + "/?a=photo&id=" + id)
    ...
    function getExhibition(id) {
      return $http
        .get($rootScope.ApiUrl + "/?a=exhibition&id=" + id)
        .success(function(data) {
          return data;
        });
    }
    ...
    function getLiterature(id) {
      return $http
        .get($rootScope.ApiUrl + "/?a=literature&id=" + id)
        .success(function(data) {
          return data;
        });
    }
