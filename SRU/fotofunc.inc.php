...
{  // expandiert Verweise in Literatur, formatiert
    $suchmuster = "/<llink:(.\d+)>/";
...
function formldatesimp($gdate, $gcode, $tdate, $tcode)
...
{ // formatiert lebenszeit in Detail
    if (!empty($go)) {
        $go = ' ' . $go;
        $go2 = '*' . $go;
    }
...
function normfeldg($tpl, $label, $value, $g)
{  //  rot darstellen wenn g=1
    if ($value != '') {
        $tpl->assign("label", $label);
        $tpl->assign('g', ($g == 1 ? 'g' : ''));
        $tpl->assign("value", $value);
...
function datumtest()
{ // testet $REQUEST vondatum und bisdatum auf falsch formatierung
