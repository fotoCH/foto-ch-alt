<?php
	...
	if(auth_level($USER_WORKER)) $def->assign("idd",$id);
	...
	$def->assign("KOMMA1", ", "); //assign komma for name, vorname in header
		...
		while($fetch=mysql_fetch_array($result)){
			...
			while($fetch4=mysql_fetch_array($result4)){
				...
				if (auth_level($USER_WORKER)) $fetch4['idf']="(id=$fetch4[id])";
				if (auth_level($USER_WORKER)){
			...
			if (auth_level($USER_WORKER)){ // alte bestaende
			...
			if (auth_level($USER_WORKER)){  //alte Literatur
